...
public class StuckWoodenKidArrowFeatureRenderer<T extends LivingEntity, M extends PlayerEntityModel<T>> extends StuckObjectsFeatureRenderer<T, M> {
    ...
    public StuckWoodenKidArrowFeatureRenderer(EntityRendererFactory.Context context, LivingEntityRenderer<T, M> entityRenderer) {
    ...
    protected int getObjectCount(T entity) {
        return ((LivingEntityInterface)entity).getStuckWoodenKidArrowCount();
    }
    ...
    protected void renderObject(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, Entity entity, float directionX, float directionY, float directionZ, float tickDelta) {
        ...
        WoodenKidArrowEntity arrowEntity = new WoodenKidArrowEntity(entity.getWorld(), entity.getX(), entity.getY(), entity.getZ(), ItemStack.EMPTY, null);
