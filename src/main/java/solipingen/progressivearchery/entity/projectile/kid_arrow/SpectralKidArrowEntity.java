...
public class SpectralKidArrowEntity extends KidArrowEntity {
    ...
    public SpectralKidArrowEntity(World world, double x, double y, double z, ItemStack stack, @Nullable ItemStack shotFrom) {
        super(ModEntityTypes.SPECTRAL_KID_ARROW_ENTITY, world, x, y, z, stack, shotFrom, DAMAGE_AMOUNT);
    }

    public SpectralKidArrowEntity(World world, LivingEntity owner, ItemStack stack, @Nullable ItemStack shotFrom) {
        super(ModEntityTypes.SPECTRAL_KID_ARROW_ENTITY, world, owner, stack, shotFrom, DAMAGE_AMOUNT);
    }
    ...
    public void tick() {
        ...
        if (world.isClient && !this.isInGround()) {
            world.addParticleClient(ParticleTypes.INSTANT_EFFECT, this.getX(), this.getY(), this.getZ(), 0.0, 0.0, 0.0);
        }
    }
