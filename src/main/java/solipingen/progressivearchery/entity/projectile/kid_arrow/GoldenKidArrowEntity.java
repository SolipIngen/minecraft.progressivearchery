...
public class GoldenKidArrowEntity extends KidArrowEntity {
    ...
    public GoldenKidArrowEntity(World world, double x, double y, double z, ItemStack stack, @Nullable ItemStack shotFrom) {
        super(ModEntityTypes.GOLDEN_KID_ARROW_ENTITY, world, x, y, z, stack, shotFrom, DAMAGE_AMOUNT);
        this.setSound(ModSoundEvents.COPPER_GOLDEN_ARROW_HIT);
    }

    public GoldenKidArrowEntity(World world, LivingEntity owner, ItemStack stack, @Nullable ItemStack shotFrom) {
        super(ModEntityTypes.GOLDEN_KID_ARROW_ENTITY, world, owner, stack, shotFrom, DAMAGE_AMOUNT);
        this.setSound(ModSoundEvents.COPPER_GOLDEN_ARROW_HIT);
    }
    ...
    public void tick() {
        ...
        if (world.isClient) {
            ...
            this.setStack(new ItemStack(ModItems.GOLDEN_KID_ARROW));
        }
    }
    ...
    private void spawnParticles(int amount) {
        ...
        if (i != -1 && amount > 0) {
            for(int j = 0; j < amount; ++j) {
                this.getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, i),
    ...
    public void handleStatus(byte status) {
        if (status == 0) {
            ...
            if (i != -1) {
                ...
                for (int j = 0; j < 20; ++j) {
                    this.getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h),
                            this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0, 0.0, 0.0);
                }
            }
        }
