...
public abstract class BowAttackGoalMixin<T extends HostileEntity> extends Goal {
    ...
    private void injectedIsHoldingBow(CallbackInfoReturnable<Boolean> cbireturn) {
        if (this.actor.isHolding(stack -> stack.isOf(Items.BOW) || stack.getItem() instanceof ModBowItem)) {
            cbireturn.setReturnValue(true);
        }
    }

    @ModifyConstant(method = "tick", constant = @Constant(intValue = 20), slice = @Slice(from = @At(value = "INVOKE", target = "Lnet/minecraft/entity/mob/HostileEntity;isUsingItem()Z"), to = @At("TAIL")))
    ...
    private void redirectedClearActiveItemTick(HostileEntity actorHostileEntity) {
        // Empty replacing method removing the clearActiveItem() to be moved to the end of the redirectedAttackTick below.
    }
    ...
    private void redirectedAttackTick(RangedAttackMob rangedAttackMob, LivingEntity livingEntity, float pullProgress) {
        ItemStack stack = this.actor.getMainHandStack();
        Item item = stack.getItem();
        int time = this.actor.getItemUseTime();
        if (item instanceof ModBowItem) {
            ((RangedAttackMob)this.actor).shootAt(livingEntity, ((ModBowItem)item).getPullProgress(time, stack, this.actor.getWorld()));
        }
        else {
            ((RangedAttackMob)this.actor).shootAt(livingEntity, ModBowItem.getVanillaBowPullProgress(time, stack, this.actor.getWorld()));
        }
    ...
    private void injectedTick(CallbackInfo cbi) {
        if (!this.actor.isUsingItem() && (--this.cooldown <= 0 && this.targetSeeingTicker >= -60)) {
            ...
            this.actor.setCurrentHand(ProjectileUtil.getHandPossiblyHolding(this.actor, handStack.getItem()));
        }
    }
