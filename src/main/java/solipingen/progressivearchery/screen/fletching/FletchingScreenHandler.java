...
public class FletchingScreenHandler extends ArrowmakingScreenHandler {
    ...
    protected boolean canTakeOutput(PlayerEntity player, boolean present) {
        ...
        return (this.currentRecipe != null && this.currentRecipe.value().matches(this.input, this.world)) || outputStack.getItem() instanceof ModArrowItem || outputStack.getItem() instanceof KidArrowItem;
    }
    ...
    protected boolean isUsableAsHead(ItemStack stack) {
        return this.recipes.stream().anyMatch(recipe -> recipe.value().testHead(stack));
    }

    @Override
    protected boolean isUsableAsBody(ItemStack stack) {
        return this.recipes.stream().anyMatch(recipe -> recipe.value().testBody(stack));
    }

    @Override
    protected boolean isUsableAsTail(ItemStack stack) {
        return this.recipes.stream().anyMatch(recipe -> recipe.value().testTail(stack));
    }

    @Override
    protected boolean isUsableAsAddition(ItemStack stack) {
        return this.recipes.stream().anyMatch(recipe -> recipe.value().testAddition(stack));
    }
