...
public class PotionCauldronBlock extends AbstractCauldronBlock {
    ...
    public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity, EntityCollisionHandler handler) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            handler.addPreCallback(CollisionEvent.EXTINGUISH, collidedEntity -> {
                if (collidedEntity.canModifyAt(serverWorld, blockPos)) {
                    ...
                    if (entity instanceof LivingEntity livingEntity) {
                        for (StatusEffectInstance statusEffectInstance : POTION_TYPE_IN_BLOCK.get(blockpotionType).getEffects()) {
                            livingEntity.addStatusEffect(new StatusEffectInstance(statusEffectInstance.getEffectType(),
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        int color = (new PotionContentsComponent(potionEntry)).getColor();
        int level = state.get(LEVEL);
        ...
        for (int j = 0; j < level; ++j) {
            world.addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, color), pos.getX() + 0.5, pos.getY() + 15.0/16.0, pos.getZ() + 0.5, d, e, f);
        }
    }
