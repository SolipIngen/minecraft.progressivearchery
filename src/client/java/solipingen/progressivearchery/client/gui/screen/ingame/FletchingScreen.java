...
//        RenderSystem.setShaderTexture(0, TEXTURE);
        ...
        if (bodySlot.getStack().getItem() instanceof ModArrowItem) {
            if (bodySlot.getStack().isIn(ModItemTags.CUTTABLE_METAL_ARROWS)) {
                context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 48, 16, 16, 256, 256);
            }
            else {
                context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 0, 16, 16, 256, 256);
            }
        }
        else {
            if (headSlot.getStack().isOf(Items.STICK)) {
                ...
                context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 16, 16, 16, 256, 256);
            }
            else if (headSlot.getStack().isIn(ModItemTags.CRAFTED_ARROWHEADS)) {
                context.drawTexture(RenderPipelines.GUI_TEXTURED, TEXTURE, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 32, 16, 16, 256, 256);
            }
        }
    }
