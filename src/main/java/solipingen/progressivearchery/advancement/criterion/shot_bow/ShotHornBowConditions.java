...
public record ShotHornBowConditions(Optional<LootContextPredicate> player, Optional<ItemPredicate> item) implements AbstractCriterion.Conditions {
    ...
    public static AdvancementCriterion<ShotHornBowConditions> create(ItemConvertible item) {
        return ModCriteria.SHOT_HORN_BOW.create(new ShotHornBowConditions(Optional.empty(), Optional.of(ItemPredicate.Builder.create().items(item).build())));
    }
