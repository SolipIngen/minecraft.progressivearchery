...
public class ModCrossbowItem extends RangedWeaponItem {
    ...
    private static final ModCrossbowItem.LoadingSounds DEFAULT_LOADING_SOUNDS = new ModCrossbowItem.LoadingSounds(Optional.of(SoundEvents.ITEM_CROSSBOW_LOADING_START), Optional.of(SoundEvents.ITEM_CROSSBOW_LOADING_MIDDLE), Optional.of(SoundEvents.ITEM_CROSSBOW_LOADING_END));


    public ModCrossbowItem(BowMaterial material, Item.Settings settings) {
        super(settings.maxDamage(MathHelper.ceil(1.15f*material.durability())).enchantable(material.enchantanmentValue()));
        this.material = material;
        this.arrowMaxReleaseSpeed = 3.6f + 0.2f*this.material.level();
        this.rocketMaxReleaseSpeed = 2.8f + 0.2f*this.material.level();
        this.divergence = 0.75f - 0.1f*this.material.level();
    }
    ...
    public boolean onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        if (world instanceof ServerWorld) {
            ...
            return ModCrossbowItem.getPullProgress(i, stack, user) >= 1.0f && ModCrossbowItem.isCharged(stack);
        }
        return false;
    }
    ...
    protected void shoot(LivingEntity shooter, ProjectileEntity projectile, int index, float speed, float divergence, float yaw, @Nullable LivingEntity target) {
        ...
        if (target != null) {
            ...
            double g = target.getBodyY(0.3333333333333333) - projectile.getY() + f*(0.10 - 0.01*this.material.level());
    ...
    protected int getWeaponStackDamage(ItemStack projectile) {
        ...
        boolean bl2 = bl && this.material.level() < BowMaterial.COPPER.level();
        boolean bl3 = bl && this.material.level() < BowMaterial.IRON.level();
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (!world.isClient()) {
            ...
            float f = (float)(stack.getMaxUseTime(user) - remainingUseTicks) / (float)ModCrossbowItem.getPullTime(stack, user);
            if (f < 0.2f) {
                this.charged = false;
                this.loaded = false;
            }
            if (f >= 0.2f && !this.charged) {
                this.charged = true;
                loadingSounds.start().ifPresent(sound -> world.playSound(null, user.getX(), user.getY(), user.getZ(), sound.value(), user.getSoundCategory(), 0.5f, 1.0f / (world.getRandom().nextFloat() * 0.5f + 1.0f) + 0.2f));
            }
            if (f >= 0.5f && !this.loaded) {
                this.loaded = true;
                loadingSounds.mid().ifPresent(sound -> world.playSound(null, user.getX(), user.getY(), user.getZ(), sound.value(), user.getSoundCategory(), 0.5f, 1.0f / (world.getRandom().nextFloat() * 0.5f + 1.0f) + 0.2f));
            }
    ...
    public int getMaxUseTime(ItemStack stack, LivingEntity user) {
        return ModCrossbowItem.getPullTime(stack, user) + 3;
    }
    ...
    private ModCrossbowItem.LoadingSounds getLoadingSounds (ItemStack stack) {
        return EnchantmentHelper.getEffect(stack, ModEnchantmentEffectComponentTypes.MOD_CROSSBOW_CHARGING_SOUNDS).orElse(DEFAULT_LOADING_SOUNDS);
    }
...
//    }
    ...
    public static record LoadingSounds(Optional<RegistryEntry<SoundEvent>> start, Optional<RegistryEntry<SoundEvent>> mid, Optional<RegistryEntry<SoundEvent>> end) {
        public static final Codec<ModCrossbowItem.LoadingSounds> CODEC = RecordCodecBuilder.create((instance) ->
                instance.group(SoundEvent.ENTRY_CODEC.optionalFieldOf("start").forGetter(ModCrossbowItem.LoadingSounds::start),
                            SoundEvent.ENTRY_CODEC.optionalFieldOf("mid").forGetter(ModCrossbowItem.LoadingSounds::mid),
                            SoundEvent.ENTRY_CODEC.optionalFieldOf("end").forGetter(ModCrossbowItem.LoadingSounds::end))
                        .apply(instance, ModCrossbowItem.LoadingSounds::new));
