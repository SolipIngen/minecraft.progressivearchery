...
public record ShotTubularBowConditions(Optional<LootContextPredicate> player, Optional<ItemPredicate> item) implements AbstractCriterion.Conditions {
    ...
    public static AdvancementCriterion<ShotTubularBowConditions> create(ItemConvertible item) {
        return ModCriteria.SHOT_TUBULAR_BOW.create(new ShotTubularBowConditions(Optional.empty(), Optional.of(ItemPredicate.Builder.create().items(item).build())));
    }
