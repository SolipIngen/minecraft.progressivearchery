...
public class FletcherBlock extends BlockWithEntity {
    ...
    protected void craft(BlockState state, ServerWorld world, BlockPos pos) {
        ...
        if (var5 instanceof FletcherBlockEntity fletcherBlockEntity) {
            Optional optional = FletcherBlock.getFletchingRecipe(world, fletcherBlockEntity);
            if (optional.isEmpty()) {
                ...
                ItemStack itemStack = (recipeEntry.value()).craft(fletcherBlockEntity, world.getRegistryManager());
                if (itemStack.isEmpty()) {
                    ...
                    Iterator<ItemStack> var8 = recipeEntry.value().getRemainder(fletcherBlockEntity).iterator();
                    while (var8.hasNext()) {
    ...
    public static Optional<RecipeEntry<FletchingRecipe>> getFletchingRecipe(World world, RecipeInputInventory inputInventory) {
...
//            Iterator<ServerPlayerEntity> var12 = world.getNonSpectatingEntities(ServerPlayerEntity.class, Box.of(vec3d, 17.0, 17.0, 17.0)).iterator();
//            while (var12.hasNext()) {
