    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        if (bodySlot.getStack().getItem() instanceof ModArrowItem) {
            if (bodySlot.getStack().isOf(ModItems.WOODEN_ARROW) || bodySlot.getStack().isOf(ModItems.FLINT_ARROW)) {
                this.drawTexture(matrices, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 0, 16, 16);
            }
            else {
                this.drawTexture(matrices, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 48, 16, 16);
            }
        }
        else {
            if (headSlot.getStack().isOf(Items.STICK)) {
                ...
                this.drawTexture(matrices, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 16, 16, 16);
            }
                ...
                this.drawTexture(matrices, i + additionSlot.x, j + additionSlot.y, this.backgroundWidth + 1, 32, 16, 16);
            }
        }
    }
